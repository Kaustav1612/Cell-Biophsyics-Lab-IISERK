function [kymograph_position,kymograph_intensity,particle_kymograph,dist_from_centroid] = ...
    generate_kymograph(image_stack,binary_stack,x_coords,y_coords,all_stats,nframes,minimum_particle_size,max_particles,num_particles_detected,current_frame,particle_presence,X_Centroids,Y_Centroids,frame_centroid)
...
y_round = round(Y_Centroids);
    ...
    for j = 1:max_particles
        for i = 1:nframes
            ...
            particle_centroid=[x_round(j,i),y_round(j,i)];
            intensity_frame = double(image_stack(:,:, i));           
           if actual_n_particles>=j
               if ~isnan(y_round(j,i)) &&  ~isnan(x_round(j,i)) 
                    dist_from_centroid(i,j)=pdist2(frame_centroid,particle_centroid);
