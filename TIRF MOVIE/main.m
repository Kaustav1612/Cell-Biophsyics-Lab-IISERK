...
olddir=cd('K:\Jibitesh\TIRF\041223\Dish1\Exported\control_001\set');
directory = "D:\Kaustav\CODES SET\aa";
...
redrawing = true;
while redrawing
  imshow(image_stack (:,:,1),[0 180]);  % Display the black and white image
...
max_merge_gap = 3;          % Maximum allowed frame gap between trajectories
...
params = struct();
params.MinArea = 10;          % Minimum object area (pixels)
params.MaxArea = 600;        % Maximum object area (pixels)
params.FrameStart = 1;        % First frame to process
params.FrameEnd = nframes;         % Last frame to process
...
close all;
pixel = 0.065;
X_Centroids_val = X_Centroids*pixel;
Y_Centroids_val = Y_Centroids *pixel;
del_t=0.2 ; 
a = image_stack(:,:,1);
TAMSD =NaN(size(nframes,max_particles));
intensity  = ones(size(nframes,max_particles))*mean(a(:));
TAMSD_z = NaN(size(nframes,max_particles));

for j = 1 : max_particles
    N = size(find(particle_presence(:,j)==1),1);
    for n  = 1 : N
        frame = image_stack(:,:,n);
        if ~isnan(Y_Centroids(j,n)) && ~isnan(X_Centroids(j,n))
            intensity(n,j) =  frame(round(Y_Centroids(j,n)),round(X_Centroids(j,n)));
        end
    end
end
...
for j = 1 : max_particles
    ...
    for n  = 1 : N
        ...
        sum_Z =0;
        for t = 1:n
            sum_X = sum_X+(X_Centroids_val(j,t) - X_Centroids_val(j, min(find(particle_presence(:,j)==1))))^2;
            sum_Y = sum_Y+(Y_Centroids_val(j,t) - Y_Centroids_val(j, min(find(particle_presence(:,j)==1))))^2;
            sum_Z = sum_Z + (intensity(t,j) - intensity(min(find(particle_presence(:,j)==1)),j))^2;
        end
        if sum_X+sum_Y ~= 0
            TAMSD(n,j)=(sum_X+sum_Y)/((N-n+1)*del_t);
            TAMSD_z(n,j) = (sum_Z)/((N-n+1)*del_t);            
...
D = zeros(max_particles,1);
particle_sizes = [];
for j =1 : max_particles
     if length(find(TAMSD(:,j)>0)) > 80
         idxs = find(find(TAMSD(:,j)>0));
         total_disp = TAMSD(idxs(end),j);
         N = idxs(end)-idxs(1);
         MSD(j,1) = total_disp;
         D(j,1) = total_disp/(4*N*del_t); % micro meter^2/s
     end
end
...
del_t = 0.2;       % Ensure this matches your actual time step
% Assuming TAMSD and max_particles exist in your workspace
% If not, uncomment below for testing:
% TAMSD = rand(100, 5) + (1:100)'.^0.8; max_particles = 5;

% --- Setup ---
% Time vector (Time Lags)
t_lags = (1:size(TAMSD, 1))' * del_t; 

% Prepare a cell array to store results (since lengths might differ after cleaning)
rolling_results = cell(max_particles, 1);
...
for j = 1:max_particles
    ...
    y_data = TAMSD(:, j);
    z_data = TAMSD_z(:,j);
    % Find indices where TAMSD is positive (valid for log)
    valid_idx = find(y_data > 0);
    valid_idx_z = find(z_data > 0);

    % Skip if particle has too little data for even one window
    if length(valid_idx) < window_size
        fprintf('Particle %d skipped: insufficient data.\n', j);
        continue;
    end
    
    % Extract valid subsets
    t_valid = t_lags(valid_idx);
    y_valid = y_data(valid_idx);
    z_valid = z_data(valid_idx_z);
    % Initialize vectors for this particle
    num_windows = length(t_valid) - window_size + 1;
    alpha_local = zeros(num_windows, 1);
    alpha_z = zeros(num_windows, 1);
    D_local = zeros(num_windows, 1);
    t_window_center = zeros(num_windows, 1);
    ...
    for k = 1:num_windows
        ...
        % Extract X (time) and Y (MSD) for this window
        t_win = t_valid(win_start:win_end);
        y_win = y_valid(win_start:win_end);
        z_win = z_valid(win_start:win_end);
        % Perform Linear Fit in Log-Log space
        % Model: log(TAMSD) = alpha * log(t) + log(4D)
        % Model: log(TAMSD_z) = alpha * log(t) + log(2D)
        % Using natural log (log) for both ensures correct slope
        p1 = polyfit(log(t_win), log(y_win), 1);
        p2 = polyfit(log(t_win), log(z_win), 1);
        val = (y_win(1) - y_win(end))/(4*(t_win(1) - t_win(end)));
        % Store the slope (alpha)
        alpha_local(k) = p1(1);
        alpha_z(k) = p2(1);
        D_local(k)=val;
        % Store the time point representing this window (e.g., the center time)
        t_window_center(k) = mean(t_win);
    end
    
    % Store results in structure for later use if needed
    rolling_results{j}.time = t_window_center;
    rolling_results{j}.alpha = alpha_local;
    rolling_results{j}.z_alpha = alpha_z;
    
    % Time vector calculation
    time = (1:size(TAMSD, 1))' * del_t;  
    % Find indices where intensity is non-zero
    valid_idx = find(intensity(:,j) > 0);
           
    % 3. Plotting on the left y axis
    % Plotting Alpha vs Time Lag 
    yyaxis left
    scatter(t_window_center, D_local, 'b', 'LineWidth', 1);
    hold on;
%     yline(1, 'k--'); % Reference line
    ylabel('Exponent \alpha');
   
%     % Plotting on the right y axis
    yyaxis right
    plot(time, 500-intensity(:,j), 'r', 'LineWidth', 1.5);
    hold on;
    ylabel('Intensity (units)');     
    title(sprintf('Particle %d', j));
...
save(results_file_name, 'kymograph_stitched_intensity', 'kymograph_stitched', 'total_lifetime_normalized','TAMSD','MSD','D','new_stats','trajectories');
cd(olddir);
